...
const DEFAULT_CONTAS = [
  ...
  {id:"cr1", nome:"Cartão Nubank", tipo:"credito",      saldoInicial:0, limite:5000, cor:"#C0616A", diaVencimento:10, diaFechamento:3},
  {id:"inv1",nome:"Investimentos", tipo:"investimento", saldoInicial:12000, cor:"#3D9E7A"},
];
const DEFAULT_TX = [
  {id:1, descricao:"AdSense Fevereiro",  valor:1200, tipo:"receita", categoria:"AdSense",      contaId:"cc1", segmento:"canal",   data:"2026-02-10", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:2, descricao:"Microfone Blue Yeti",valor:650,  tipo:"despesa", categoria:"Equipamento",  contaId:"cr1", segmento:"canal",   data:"2026-02-08", nota:"Para podcast", parcelaAtual:0, totalParcelas:0},
  {id:3, descricao:"Salário",            valor:3500, tipo:"receita", categoria:"Salário",      contaId:"cc1", segmento:"pessoal", data:"2026-02-05", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:4, descricao:"Aluguel",            valor:900,  tipo:"despesa", categoria:"Moradia",      contaId:"cc1", segmento:"pessoal", data:"2026-02-01", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:5, descricao:"Aporte Mensal",      valor:500,  tipo:"despesa", categoria:"Outros Pessoal",contaId:"inv1",segmento:"pessoal", data:"2026-02-03", nota:"Tesouro Direto", parcelaAtual:0, totalParcelas:0},
  {id:6, descricao:"Patrocínio TikTok",  valor:800,  tipo:"receita", categoria:"Patrocínio",   contaId:"cc1", segmento:"canal",   data:"2026-02-15", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:7, descricao:"Adobe Premiere",     valor:120,  tipo:"despesa", categoria:"Edição",       contaId:"cr1", segmento:"canal",   data:"2026-02-12", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:8, descricao:"Mercado",            valor:430,  tipo:"despesa", categoria:"Alimentação",  contaId:"cr1", segmento:"pessoal", data:"2026-02-18", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:9, descricao:"AdSense Janeiro",    valor:980,  tipo:"receita", categoria:"AdSense",      contaId:"cc1", segmento:"canal",   data:"2026-01-10", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:10,descricao:"Salário Jan",        valor:3500, tipo:"receita", categoria:"Salário",      contaId:"cc1", segmento:"pessoal", data:"2026-01-05", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:11,descricao:"Aluguel Jan",        valor:900,  tipo:"despesa", categoria:"Moradia",      contaId:"cc1", segmento:"pessoal", data:"2026-01-01", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:12,descricao:"Mercado Jan",        valor:380,  tipo:"despesa", categoria:"Alimentação",  contaId:"cr1", segmento:"pessoal", data:"2026-01-20", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:13,descricao:"Patrocínio Jan",     valor:600,  tipo:"receita", categoria:"Patrocínio",   contaId:"cc1", segmento:"canal",   data:"2026-01-18", nota:"", parcelaAtual:0, totalParcelas:0},
  {id:14,descricao:"Adobe Jan",          valor:120,  tipo:"despesa", categoria:"Edição",       contaId:"cr1", segmento:"canal",   data:"2026-01-12", nota:"", parcelaAtual:0, totalParcelas:0},
];
const DEFAULT_ORC = [
...
function EmojiPicker({ value, onChange }) {
  ...
  const [catAtiva, setCatAtiva] = useState(Object.keys(EMOJI_CATS)[0]);
  return (
    <div style={{ position:"relative" }}>
      <div style={{ display:"flex", gap:8, alignItems:"center" }}>
        <button type="button" onClick={()=>setOpen(o=>!o)} style={{
          width:48, height:48, borderRadius:12, border:"1px solid #DDD8CF",
          background:"#FAF9F6", fontSize:24, cursor:"pointer", display:"flex",
          alignItems:"center", justifyContent:"center", flexShrink:0,
          transition:"border-color .15s",
        }}>{value || "➕"}</button>
        <span style={{fontSize:12,color:"#A09080"}}>Clique para escolher o ícone</span>
      </div>
      {open && (
        <div style={{
          position:"absolute", top:54, left:0, zIndex:300,
          background:"#FFF", border:"1px solid #E8E3DB", borderRadius:16,
          boxShadow:"0 8px 32px #00000018", padding:16, width:300,
        }}>
          {/* Categoria tabs */}
          <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginBottom:12 }}>
            {Object.keys(EMOJI_CATS).map(cat => (
              <button key={cat} onClick={()=>setCatAtiva(cat)} style={{
                ...
                background: catAtiva===cat ? "#3D9E7A12" : "transparent",
                color: catAtiva===cat ? "#3D9E7A" : "#A09080",
                transition:"all .15s",
              }}>{cat}</button>
            ))}
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:4 }}>
            {EMOJI_CATS[catAtiva].map(e => (
              <button key={e} onClick={()=>{ onChange(e); setOpen(false); }} style={{
                border:"none", background: value===e ? "#3D9E7A18" : "transparent",
                borderRadius:8, padding:6, fontSize:20, cursor:"pointer",
                transition:"background .1s", lineHeight:1,
              }}>{e}</button>
            ))}
          </div>
          <button onClick={()=>setOpen(false)} style={{
            marginTop:10, width:"100%", border:"none", background:"#F7F5F1",
            borderRadius:8, padding:"6px", fontSize:12, color:"#A09080",
...
// ── Cartão summary for installments ──────────────────────────────────────────
function ResumoParcelamentos({ transactions, contas }) {
  const hoje = new Date();
  const mesAtual = `${hoje.getFullYear()}-${String(hoje.getMonth()+1).padStart(2,"0")}`;

  // group by parcelaGrupoId, show only current-month slice
  const parcelas = transactions.filter(t =>
    t.totalParcelas > 1 &&
    t.tipo === "despesa" &&
    t.data.slice(0,7) >= mesAtual
  );

  // Deduplicate: one row per grupoId showing next upcoming parcela
  const grupos = {};
  parcelas.forEach(t => {
    const g = t.parcelaGrupoId;
    if (!g) return;
    if (!grupos[g] || t.data < grupos[g].data) grupos[g] = t;
  });
  ...
  return (
    <div className="card" style={{borderLeft:"3px solid #C0616A", background:"#FFF8F8"}}>
      <Label>💳 Próximas Parcelas no Cartão</Label>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {lista.map(t => {
          const conta = contas.find(c=>c.id===t.contaId);
          return (
            <div key={t.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:"#FAF9F6",borderRadius:10,flexWrap:"wrap",gap:6}}>
              <div>
                <div style={{fontWeight:500,fontSize:14}}>{t.descricao}</div>
                <div style={{fontSize:12,color:"#A09080"}}>{conta?.nome} · Vence {t.data} · Parcela {t.parcelaAtual}/{t.totalParcelas}</div>
              </div>
              <span style={{fontFamily:"'Playfair Display',serif",fontSize:17,color:CORES.despesa}}>-{fmt(t.valor)}</span>
            </div>
          );
        })}
      </div>
...
export default function ControleFinanceiro() {
  ...
  // Forms
  const emptyTx = {descricao:"",valor:"",tipo:"despesa",categoria:"",contaId:contas[0]?.id||"",segmento:"pessoal",data:new Date().toISOString().slice(0,10),nota:"",parcelado:false,numParcelas:2,parcelaAtual:0,totalParcelas:0};
  const emptyConta = {nome:"",tipo:"corrente",saldoInicial:"",limite:"",cor:"#5B8FCC",diaVencimento:"",diaFechamento:""};
  ...
  // Quick-update canal modal state
  const [modalCanal, setModalCanal] = useState(false);
  const [quickCanal, setQuickCanal] = useState({});
  ...
  const openModal = (type, item=null) => {
    ...
    if(type==="tx")         setFormTx(item?{...item,parcelado:false}:{...emptyTx,contaId:contas[0]?.id||""});
  ...
  // ── Quick canal update ────────────────────────────────────────────────────
  const openQuickCanal = () => {
    const ultimo = canalData[canalData.length-1] || {};
    setQuickCanal({...ultimo, mes:""});
    setModalCanal(true);
  };
  const saveQuickCanal = () => {
    if (!quickCanal.mes) return;
    const entry = {
      mes:      quickCanal.mes,
      inscritos:parseInt(quickCanal.inscritos)||0,
      views:    parseInt(quickCanal.views)||0,
      rpm:      parseFloat(quickCanal.rpm)||0,
      receita:  parseFloat(quickCanal.receita)||0,
    };
    setCanalData(p => {
      const idx = p.findIndex(x=>x.mes===entry.mes);
  ...
  // ── Save TX (with installments) ───────────────────────────────────────────
  const saveTx = () => {
    if (!formTx.descricao||!formTx.valor||!formTx.categoria) return;
    const base = parseFloat(formTx.valor);

    if (formTx.parcelado && formTx.tipo==="despesa" && formTx.numParcelas>1) {
      const n      = parseInt(formTx.numParcelas);
      const parcVal = parseFloat((base/n).toFixed(2));
      const grupoId = "pg_"+Date.now();
      const novos   = Array.from({length:n},(_,i)=>({
        ...
        data:         addMonths(formTx.data, i),
        descricao:    formTx.descricao,
        nota:         formTx.nota,
      }));
      setTransactions(p=>[...p,...novos]);
    } else {
      const tx = {...formTx, valor:base, parcelaAtual:0, totalParcelas:0};
  ...
  const saveConta = ()=>{
    ...
    const c={...formConta,saldoInicial:parseFloat(formConta.saldoInicial)||0,limite:parseFloat(formConta.limite)||0,diaVencimento:parseInt(formConta.diaVencimento)||0,diaFechamento:parseInt(formConta.diaFechamento)||0};
  ...
  const aplicarRecorrentes = ()=>{
    ...
    const novos=recorrentes.filter(r=>r.ativo).map(r=>({
      ...r,id:Date.now()+Math.random(),nota:"🔄 Recorrente",parcelaAtual:0,totalParcelas:0,
  ...
  const exportPDF = ()=>{
    ...
    const rows=[...transactions].sort((a,b)=>b.data.localeCompare(a.data)).map(t=>{
      const conta=contas.find(c=>c.id===t.contaId);
      const parc=t.totalParcelas>1?` (${t.parcelaAtual}/${t.totalParcelas})`:"";
      return `<tr><td>${t.data}</td><td>${t.descricao}${parc}</td><td>${t.categoria}</td><td>${conta?.nome||"—"}</td><td>${t.tipo}</td><td style="color:${t.tipo==="receita"?"#2A7A5C":"#9A3E46"};font-weight:600">${t.tipo==="receita"?"+":"-"}${fmt(t.valor)}</td></tr>`;
    }).join("");
    w.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>FinFlow PDF</title>
    <style>body{font-family:'Segoe UI',sans-serif;color:#2D2D2D;padding:32px;max-width:960px;margin:0 auto}
    h1{color:#3D9E7A}h2{font-size:16px;margin:24px 0 10px;color:#5A5040}
    .grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin:20px 0}
    .card{background:#F7F5F1;border-radius:12px;padding:16px;border-top:3px solid #ddd}
    .val{font-size:22px;font-weight:700;margin:4px 0}.lbl{font-size:11px;color:#A09080;text-transform:uppercase;letter-spacing:1px}
    table{width:100%;border-collapse:collapse;font-size:13px}th{text-align:left;padding:8px 10px;color:#A09080;font-weight:500;border-bottom:2px solid #E8E3DB}
    td{padding:8px 10px;border-bottom:1px solid #F0EDE7}</style></head><body>
  ...
  const periodoLabel = filtroPeriodo.ativo
    ? filtroPeriodo.mes ? `${MESES_NOMES[parseInt(filtroPeriodo.mes)-1]}/${filtroPeriodo.ano}` : `Ano ${filtroPeriodo.ano}`
    : "Todos os períodos";
  const anosDisponiveis = [...new Set(transactions.map(t=>t.data.slice(0,4)))].sort();
  const contasCredito   = contas.filter(c=>c.tipo==="credito");
  const cats    = allCats[formTx.segmento]||[];
  const catsRec = allCats[formRec.segmento]||[];

  // Is selected account a credit card?
  const contaSelecionadaCredito = contas.find(c=>c.id===formTx.contaId)?.tipo==="credito";
    ...
    <div style={{minHeight:"100vh",background:"#F7F5F1",fontFamily:"'DM Sans',sans-serif",color:"#2D2D2D"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .btn-icon{background:none;border:none;cursor:pointer;padding:4px 8px;border-radius:6px;color:#A09080;font-size:14px;font-family:inherit}.btn-icon:hover{background:#EDE9E2;color:#5A5040}
        ...
        input[type=range]{background:transparent;border:none;box-shadow:none;padding:4px 0;accent-color:#3D9E7A}
        ...
        .tag-parcela{background:#5B8FCC18;color:#3A6EB0}
        ...
        .modal{background:#FFF;border:1px solid #E8E3DB;border-radius:20px;padding:28px;width:100%;max-width:500px;box-shadow:0 24px 64px #00000014;max-height:92vh;overflow-y:auto}
        ...
        .toggle-tipo button{flex:1;padding:10px;border-radius:10px;border:1px solid #DDD8CF;background:transparent;color:#A09080;cursor:pointer;font-family:inherit;font-size:14px;font-weight:500;transition:all .15s}
        .parcela-box{background:#5B8FCC10;border:1px solid #5B8FCC30;border-radius:12px;padding:14px;margin-top:4px}
      ...
      <div style={{borderBottom:"1px solid #E8E3DB",background:"#FFF",padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
        ...
        <nav style={{display:"flex",gap:2,flexWrap:"wrap"}}>
          {[["dashboard","📊"],["lancamentos","💸"],["contas","🏦"],["metas","🎯"],["canal","📹"],["recorrentes","🔄"],["orcamentos","⚠️"],["relatorios","📈"]].map(([k,icon])=>(
            <button key={k} className={`nav-btn ${page===k?"active":""}`} onClick={()=>setPage(k)}>{icon} <span style={{textTransform:"capitalize"}}>{k}</span></button>
          ))}
        </nav>
      </div>
      ...
      <div style={{background:"#FFF",borderBottom:"1px solid #F0EDE7",padding:"8px 20px"}}>
        <div className="periodo-bar" style={{maxWidth:1100,margin:"0 auto"}}>
          ...
          {anosDisponiveis.map(a=>(
            <button key={a} className={`pill ${filtroPeriodo.ativo&&filtroPeriodo.ano===a&&!filtroPeriodo.mes?"active":""}`} onClick={()=>setFiltroPeriodo({ativo:true,ano:a,mes:""})}>
              {a}
            </button>
          ))}
          <select style={{width:"auto",padding:"5px 10px",fontSize:12,borderRadius:8}} value={filtroPeriodo.mes} onChange={e=>setFiltroPeriodo(p=>({...p,ativo:true,mes:e.target.value}))}>
      ...
      <div style={{padding:"24px",maxWidth:1100,margin:"0 auto"}}>
        ...
        {page==="dashboard" && (
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            ...
            {/* Próximas parcelas */}
            <ResumoParcelamentos transactions={transactions} contas={contas}/>
            ...
            <div className="row">
              <div className="card col" style={{borderLeft:`3px solid ${CORES.canal}`,background:"#FFFCF8",minWidth:220}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <Label style={{marginBottom:0}}>📹 Canais</Label>
                  <button className="btn-ghost" onClick={openQuickCanal} style={{fontSize:12,padding:"5px 10px"}}>⚡ Atualizar métricas</button>
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  ...
                  return (
                    <div key={t.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",background:"#FAF9F6",borderRadius:10,flexWrap:"wrap",gap:8,border:"1px solid #EEE9DF"}}>
                      <div style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:150}}>
                        ...
                        <div>
                          <div style={{fontSize:14,fontWeight:500}}>{t.descricao}{t.totalParcelas>1&&<span className="tag-parcela" style={{marginLeft:6,fontSize:11,padding:"1px 7px",borderRadius:10,background:"#5B8FCC18",color:"#3A6EB0"}}>{t.parcelaAtual}/{t.totalParcelas}</span>}</div>
        ...
        {page==="lancamentos"&&(
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div className="search-box">
              ...
              {busca&&<button className="btn-icon" onClick={()=>setBusca("")}>✕</button>}
            </div>
                  ...
                  <div key={t.id} className="card" style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
                    ...
                    <div style={{flex:1,minWidth:130}}>
                      <div style={{fontWeight:500,marginBottom:2}}>
                        {t.descricao}
                        {t.totalParcelas>1&&<span style={{marginLeft:8,fontSize:11,padding:"2px 8px",borderRadius:10,background:"#5B8FCC18",color:"#3A6EB0",fontWeight:500}}>Parcela {t.parcelaAtual}/{t.totalParcelas}</span>}
                      </div>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:7}}>
                      ...
                      {(!t.totalParcelas||t.totalParcelas<=1)&&<button className="btn-icon" onClick={()=>openModal("tx",t)}>✏️</button>}
                      <button className="btn-danger" onClick={()=>{
                        if(t.parcelaGrupoId) {
                          if(window.confirm("Remover todas as parcelas deste lançamento?")) setTransactions(p=>p.filter(x=>x.parcelaGrupoId!==t.parcelaGrupoId));
                        } else setTransactions(p=>p.filter(x=>x.id!==t.id));
                      }}>✕</button>
                    </div>
                  </div>
                );
        ...
        {page==="contas"&&(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
              <div><div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:600}}>Contas</div><div style={{color:"#A09080",fontSize:13,marginTop:2}}>{contas.length} contas</div></div>
                  ...
                  {c.tipo==="credito"&&c.limite>0&&(
                    <div style={{marginTop:10}}>
                      ...
                      {(c.diaFechamento||c.diaVencimento)&&(
                        <div style={{marginTop:8,display:"flex",gap:12,fontSize:12,color:"#A09080"}}>
                          {c.diaFechamento&&<span>📅 Fecha dia <strong style={{color:"#5A5040"}}>{c.diaFechamento}</strong></span>}
                          {c.diaVencimento&&<span>💳 Vence dia <strong style={{color:"#C0616A"}}>{c.diaVencimento}</strong></span>}
                        </div>
                      )}
                      {/* Parcelas futuras comprometidas neste cartão */}
                      {(()=>{
                        const hoje=new Date();
                        const mesAtual=`${hoje.getFullYear()}-${String(hoje.getMonth()+1).padStart(2,"0")}`;
                        const comprometido=transactions.filter(t=>t.contaId===c.id&&t.totalParcelas>1&&t.tipo==="despesa"&&t.data.slice(0,7)>mesAtual).reduce((s,t)=>s+t.valor,0);
                        if(!comprometido) return null;
                        return <div style={{fontSize:12,color:"#C0616A",marginTop:6}}>⚠️ {fmt(comprometido)} em parcelas futuras</div>;
                      })()}
                    </div>
                  )}
        ...
        {page==="metas"&&(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
              <div><div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:600}}>Metas</div><div style={{color:"#A09080",fontSize:13,marginTop:2}}>Seus objetivos financeiros</div></div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {metas.map(m=>{
                ...
                const diasRestantes=Math.max(0,Math.ceil((new Date(m.prazo)-new Date())/(1000*60*60*24)));
                return (
                  <div key={m.id} className="card" style={{borderLeft:`4px solid ${concluida?"#3D9E7A":m.cor}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14,flexWrap:"wrap",gap:8}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <div>
                          ...
                          <div style={{fontSize:12,color:"#A09080",marginTop:2}}>Prazo: {new Date(m.prazo).toLocaleDateString("pt-BR")} · {concluida?"✅ Concluída":`${diasRestantes} dias restantes`}</div>
                        </div>
                      </div>
                      <div style={{display:"flex",gap:8}}>
                        <button className="btn-ghost" onClick={()=>{const v=prompt("Adicionar valor (R$):");if(v&&!isNaN(parseFloat(v)))setMetas(p=>p.map(x=>x.id===m.id?{...x,acumulado:Math.min(x.valor,x.acumulado+parseFloat(v))}:x));}}>+ Aportar</button>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:8,flexWrap:"wrap",gap:8}}>
                      <div><span className="stat-num" style={{fontSize:22,color:concluida?"#3D9E7A":m.cor}}>{fmt(m.acumulado)}</span><span style={{fontSize:14,color:"#A09080"}}> de {fmt(m.valor)}</span></div>
                      <span className="stat-num" style={{fontSize:22,color:concluida?"#3D9E7A":m.cor}}>{pct}%</span>
                    </div>
                    <div className="bar-bg" style={{height:10}}><div className="bar" style={{width:`${pct}%`,height:10,background:concluida?"#3D9E7A":m.cor}}/></div>
                    {!concluida&&<div style={{fontSize:12,color:"#A09080",marginTop:6}}>Faltam {fmt(m.valor-m.acumulado)}</div>}
                  </div>
                );
              })}
            </div>
        ...
        {page==="canal"&&(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
              <div><div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:600}}>Canal</div><div style={{color:"#A09080",fontSize:13,marginTop:2}}>YouTube + TikTok</div></div>
              <div style={{display:"flex",gap:8}}>
                <button className="btn btn-secondary" onClick={openQuickCanal}>⚡ Atualização Rápida</button>
                <button className="btn btn-primary" onClick={()=>openModal("canal")}>+ Mês Manual</button>
              </div>
            </div>
            {canalData.length>0&&(()=>{
            ...
            <div className="card">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={canalData}>
                  ...
                  <YAxis hide/><Tooltip contentStyle={tooltipStyle}/>
                  <Legend formatter={v=><span style={{color:"#5A5040",fontSize:12}}>{v}</span>}/>
                  <Line type="monotone" dataKey="inscritos" stroke="#5B8FCC" strokeWidth={2} dot={{r:3}} name="Inscritos"/>
                  <Line type="monotone" dataKey="views" stroke={CORES.canal} strokeWidth={2} dot={{r:3}} name="Views"/>
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        {page==="recorrentes"&&(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
              <div><div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:600}}>Recorrentes</div><div style={{color:"#A09080",fontSize:13,marginTop:2}}>Despesas e receitas fixas</div></div>
              <div style={{display:"flex",gap:8}}>
        ...
        {page==="orcamentos"&&(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
              <div><div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:600}}>Orçamentos</div><div style={{color:"#A09080",fontSize:13,marginTop:2}}>Limites por categoria</div></div>
                ...
                <div key={a.categoria} className="card">
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8}}>
                    <div>
                      ...
                      <div style={{fontSize:12,color:"#A09080",marginTop:2}}>{fmt(a.gasto)} de {fmt(a.limite)}{a.pct>=100&&<span style={{color:CORES.despesa,fontWeight:600,marginLeft:8}}>⚠️ Limite atingido!</span>}{a.pct>=90&&a.pct<100&&<span style={{color:"#C0616A",fontWeight:600,marginLeft:8}}>🔴 Acima de 90%</span>}{a.pct>=70&&a.pct<90&&<span style={{color:CORES.canal,fontWeight:600,marginLeft:8}}>🟡 Atenção</span>}</div>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
        ...
        {page==="relatorios"&&(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12}}>
              <div><div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:600}}>Relatórios</div><div style={{color:"#A09080",fontSize:13,marginTop:2}}>{periodoLabel}</div></div>
            ...
            <div className="card">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <ComposedChart data={comparativoMensal}>
                  ...
                  <Bar dataKey="receitas" fill={CORES.receita} radius={[4,4,0,0]} name="Receitas" opacity={0.85}/>
                  <Bar dataKey="despesas" fill={CORES.despesa} radius={[4,4,0,0]} name="Despesas" opacity={0.85}/>
                  <Line type="monotone" dataKey="saldo" stroke="#5B8FCC" strokeWidth={2} dot={{r:3}} name="Saldo"/>
                </ComposedChart>
              </ResponsiveContainer>
            </div>
            <div className="card">
            ...
            <div className="card">
              ...
              <div style={{overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                  <thead><tr style={{borderBottom:"1px solid #E8E3DB"}}>{["Data","Descrição","Parcela","Categoria","Conta","Tipo","Valor"].map(h=><th key={h} style={{padding:"8px 12px",color:"#A09080",fontWeight:500,textAlign:"left",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
                  <tbody>
                    {[...txFiltradas].sort((a,b)=>b.data.localeCompare(a.data)).map(t=>{
                      ...
                      return (
                        <tr key={t.id} style={{borderBottom:"1px solid #F0EDE7"}}>
                          ...
                          <td style={{padding:"9px 12px"}}>{t.totalParcelas>1?<span style={{fontSize:11,padding:"2px 8px",borderRadius:10,background:"#5B8FCC18",color:"#3A6EB0"}}>{t.parcelaAtual}/{t.totalParcelas}</span>:"—"}</td>
      ...
      {/* ══ MODAL QUICK CANAL ══ */}
      {modalCanal&&(
        <div className="modal-overlay" onClick={e=>{if(e.target===e.currentTarget)setModalCanal(false)}}>
          <div className="modal">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:600}}>⚡ Atualização Rápida do Canal</div>
              <button onClick={()=>setModalCanal(false)} style={{background:"none",border:"none",color:"#A09080",cursor:"pointer",fontSize:20}}>✕</button>
            </div>
            <div style={{fontSize:13,color:"#A09080",marginBottom:16,background:"#F7F5F1",padding:"10px 14px",borderRadius:10}}>
              💡 Os campos já estão preenchidos com os valores do mês anterior. Altere só o que mudou e salve.
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:13}}>
              <div>
                <Label>Mês/Ano (ex: Mar/26)</Label>
                <input placeholder="Mar/26" value={quickCanal.mes||""} onChange={e=>setQuickCanal(p=>({...p,mes:e.target.value}))}/>
              </div>
              {[["inscritos","Inscritos totais","number"],["views","Views do mês","number"],["rpm","RPM (R$ por mil views)","number"],["receita","Receita total (R$)","number"]].map(([k,label,type])=>(
                <div key={k}>
                  <Label>{label}</Label>
                  <input type={type} step={k==="rpm"?"0.01":"1"} value={quickCanal[k]||""} onChange={e=>setQuickCanal(p=>({...p,[k]:e.target.value}))}/>
                </div>
              ))}
              <button className="btn btn-primary" onClick={saveQuickCanal} style={{padding:"13px",fontSize:15,marginTop:4}}>Salvar</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="modal">
              ...
              <div style={{display:"flex",flexDirection:"column",gap:13}}>
                <div className="toggle-tipo">
                  {[["receita","📥 Receita"],["despesa","📤 Despesa"]].map(([v,l])=>(
                    <button key={v} onClick={()=>setFormTx({...formTx,tipo:v,parcelado:false})} style={{flex:1,padding:"10px",borderRadius:10,border:`1px solid ${formTx.tipo===v?(v==="receita"?CORES.receita:CORES.despesa):"#DDD8CF"}`,background:formTx.tipo===v?(v==="receita"?"#3D9E7A12":"#C0616A12"):"transparent",color:formTx.tipo===v?(v==="receita"?CORES.receita:CORES.despesa):"#A09080",cursor:"pointer",fontFamily:"inherit",fontSize:14,fontWeight:500,transition:"all .15s"}}>{l}</button>
                  ))}
                </div>
                <div className="toggle-tipo">
                ...
                <input type="number" placeholder="Valor total (R$)" value={formTx.valor} onChange={e=>setFormTx({...formTx,valor:e.target.value})} min="0" step="0.01"/>
                ...
                {/* Conta */}
                <select value={formTx.contaId} onChange={e=>setFormTx({...formTx,contaId:e.target.value,parcelado:false})}>
                  {contas.map(c=><option key={c.id} value={c.id}>{c.nome} ({c.tipo})</option>)}
                </select>

                {/* Parcelamento — só para despesas em cartão de crédito */}
                {formTx.tipo==="despesa" && contaSelecionadaCredito && !editItem && (
                  <div>
                    <label style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer",fontSize:14,color:"#5A5040"}}>
                      <input type="checkbox" checked={formTx.parcelado} onChange={e=>setFormTx({...formTx,parcelado:e.target.checked})} style={{width:16,height:16,accentColor:"#3D9E7A",cursor:"pointer"}}/>
                      💳 Parcelar esta compra no cartão
                    </label>
                    {formTx.parcelado&&(
                      <div className="parcela-box" style={{marginTop:10}}>
                        <Label>Número de parcelas</Label>
                        <div style={{display:"flex",alignItems:"center",gap:12}}>
                          <input type="range" min="2" max="24" value={formTx.numParcelas} onChange={e=>setFormTx({...formTx,numParcelas:parseInt(e.target.value)})} style={{flex:1}}/>
                          <span style={{fontFamily:"'Playfair Display',serif",fontSize:20,color:"#5B8FCC",minWidth:30}}>{formTx.numParcelas}x</span>
                        </div>
                        {formTx.valor&&(
                          <div style={{marginTop:10,display:"flex",gap:16,flexWrap:"wrap"}}>
                            <div style={{fontSize:13,color:"#5A5040"}}>
                              Valor de cada parcela: <strong style={{color:"#5B8FCC"}}>{fmt(parseFloat(formTx.valor)/formTx.numParcelas)}</strong>
                            </div>
                            <div style={{fontSize:13,color:"#A09080"}}>
                              Total: {fmt(parseFloat(formTx.valor))}
                            </div>
                          </div>
                        )}
                        <div style={{fontSize:12,color:"#A09080",marginTop:8}}>
                          📅 Lançamentos criados automaticamente a partir de {formTx.data}, um por mês
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <button className="btn btn-primary" onClick={saveTx} style={{marginTop:2,padding:"13px",fontSize:15}}>
                  {formTx.parcelado?`Criar ${formTx.numParcelas} parcelas`:"Salvar"}
                </button>
              </div>
            )}
            ...
            {modal==="conta"&&(
              <div style={{display:"flex",flexDirection:"column",gap:13}}>
                ...
                {formConta.tipo==="credito"&&(
                  <>
                    <input type="number" placeholder="Limite do cartão (R$)" value={formConta.limite} onChange={e=>setFormConta({...formConta,limite:e.target.value})} min="0" step="0.01"/>
                    <div style={{display:"flex",gap:10}}>
                      <div style={{flex:1}}>
                        <div style={{fontSize:12,color:"#A09080",marginBottom:6}}>📅 Dia de fechamento</div>
                        <input type="number" placeholder="Ex: 3" value={formConta.diaFechamento} onChange={e=>setFormConta({...formConta,diaFechamento:e.target.value})} min="1" max="31"/>
                      </div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:12,color:"#A09080",marginBottom:6}}>💳 Dia de vencimento</div>
                        <input type="number" placeholder="Ex: 10" value={formConta.diaVencimento} onChange={e=>setFormConta({...formConta,diaVencimento:e.target.value})} min="1" max="31"/>
                      </div>
                    </div>
                    <div style={{fontSize:12,color:"#A09080",background:"#F7F5F1",padding:"8px 12px",borderRadius:8}}>
                      💡 Compras feitas após o dia de fechamento entram na fatura do próximo mês. O sistema usará essas datas para alertas de vencimento.
                    </div>
                  </>
                )}
                <div>
                ...
                <button className="btn btn-primary" onClick={saveConta} style={{padding:"13px",fontSize:15}}>Salvar</button>
              </div>
            )}
            ...
            {modal==="recorrente"&&(
              <div style={{display:"flex",flexDirection:"column",gap:13}}>
                ...
                <button className="btn btn-primary" onClick={saveRec} style={{padding:"13px",fontSize:15}}>Salvar</button>
              </div>
            )}
            ...
            {modal==="orcamento"&&(
              <div style={{display:"flex",flexDirection:"column",gap:13}}>
                ...
                <button className="btn btn-primary" onClick={saveOrc} style={{padding:"13px",fontSize:15}}>Salvar</button>
              </div>
            )}
            ...
            {modal==="meta"&&(
              <div style={{display:"flex",flexDirection:"column",gap:13}}>
                <input placeholder="Nome da meta" value={formMeta.nome} onChange={e=>setFormMeta({...formMeta,nome:e.target.value})}/>
                <div>
                  <Label>Ícone</Label>
                  <EmojiPicker value={formMeta.icone} onChange={v=>setFormMeta({...formMeta,icone:v})}/>
                </div>
                <input type="number" placeholder="Valor total (R$)" value={formMeta.valor} onChange={e=>setFormMeta({...formMeta,valor:e.target.value})} min="0" step="0.01"/>
                ...
                <div><Label>Prazo</Label><input type="date" value={formMeta.prazo} onChange={e=>setFormMeta({...formMeta,prazo:e.target.value})}/></div>
                <div>
                  <Label>Cor</Label>
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {["#3D9E7A","#5B8FCC","#8B6BB5","#C0616A","#D4894A","#6A9EB5","#7EB87E","#B5896A"].map(cor=>(
                ...
                <button className="btn btn-primary" onClick={saveMeta} style={{padding:"13px",fontSize:15}}>Salvar</button>
              </div>
            )}
          </div>
        </div>
